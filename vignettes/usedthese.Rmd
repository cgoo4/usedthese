...
In the example below, `tribble` is counted once against the (originating) tibble package even though it is also loaded by dplyr. And `used_here()` assumes a loaded function is used in preference to a base function of the same name, so `dplyr::filter`, for example, is counted rather than `stats::filter`.
