...
The package author recommends using the [conflicted package](https://conflicted.r-lib.org) to resolve conflicts. In the example below, 'dplyr' has been preferred over stats for `filter()` and `lag()`, and over the xts package for `first()` and `last()`.
